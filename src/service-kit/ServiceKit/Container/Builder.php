...
class Builder
{
    ...
    public function build(Bag $bag)
    {
        ...
        $objectContainer = new ObjectContainer($this->createObjectStorer($bag));
    ...
    private function createTagStorer(Bag $bag)
    {
        ...
        foreach ($bag->getInstances() as $instance) {
            foreach ($instance->getTags() as $tag) {
                $ids[$tag['name']][$tag['key']] = $instance->getId();
            }
        }
     ...
     * @return ObjectStorer
     */
    private function createObjectStorer(Bag $bag)
    {
        $storer = new ObjectStorer();
